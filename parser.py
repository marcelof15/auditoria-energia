...
def extrair_cosern(path):
    with pdfplumber.open(path) as pdf:
        ...
        texto_uc = page.crop((w*0.25, h*0.05, w*0.65, h*0.28)).extract_text() or ""
    r = {}
    # UC — crop central onde aparece limpo
    m = re.search(r'CÓDIGO DA INSTALAÇÃO\s*\n(\d{7})', texto_uc)
    if not m: m = re.search(r'\b(\d{7})\b', texto_uc)
    r['uc'] = m.group(1) if m else ''
    # Torre
    m = re.search(r'TORRE\s+(\w+)\s+BL', texto)
    r['torre'] = m.group(1).capitalize() if m else ''
    # Competência — rodapé é o mais confiável
    m = re.search(r'^(\d{2}/20\d{2})\s+CÓDIGO DO CLIENTE', texto, re.MULTILINE)
    ...
    # G2 — geração própria (não entra no desconto Origo)
